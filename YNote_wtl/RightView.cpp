/**
 * @file    RightView.cpp
 * @author  Zhan WANG <wangzhan.1985@gmail.com>
 */
#include "stdafx.h"
#include "RightView.h"


CRightView::CRightView() : 
    m_crText(RGB(0,0,0)),
    m_crBkgnd(RGB(153,0,255))
{

}

BOOL CRightView::PreTranslateMessage(MSG* pMsg)
{
    pMsg;
    return FALSE;
}

LRESULT CRightView::OnCreate(LPCREATESTRUCT lpcs)
{
    ATLVERIFY(m_ctrlEditorView.Create(*this, rcDefault, NULL, CHILD_WND_STYLE));

    SetMsgHandled(false);
    return 0;
}

void CRightView::OnDestroy()
{
    SetMsgHandled(false);
}

LRESULT CRightView::OnEraseBkgnd(HDC hdc)
{
    return 1;   // OnPaint will draw the whole window
}

void CRightView::OnSize(UINT uiParam, _WTYPES_NS::CSize size)
{
    m_ctrlEditorView.SetWindowPos(NULL, 0, 0, size.cx, size.cy, 0);
}

LRESULT CRightView::OnPaint(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
    CPaintDC dc(m_hWnd);
    CRect    rc;
    CString  sTime;

    // Get our window's client area.
    GetClientRect ( rc );

    // Build the string to show in the window.
    sTime.Format ( _T("Clock stopped on"));

    // Set up the DC and draw the text.
    dc.SaveDC();

    dc.SetBkColor ( m_crBkgnd );
    dc.SetTextColor ( m_crText );
    dc.ExtTextOut ( 0, 0, ETO_OPAQUE, rc, sTime, sTime.GetLength(), NULL );

    // Restore the DC.
    dc.RestoreDC(-1);
    return 0;
}
